...
process RUN_TRUE_MSA_RAXML {
    ...
    output:
    path("chunk_true_msa_raxml_D${dist}_L${len}_chk${chunk_id}.csv"), emit: csv
    path("true_msa_raxml_*.nwk")
    path("true_msa_raxml_meta_*.json")
    ...
    for rep in \$(seq ${rep_start} ${rep_end}); do
        python3 ${moduleDir}/../bin/run_msa_raxml.py \\
            ...
            --outtree true_msa_raxml_\${rep}.nwk \\
            --outjson true_msa_raxml_meta_\${rep}.json \\
            --model "${raxml_model}" \\
            --seed \${rep} \\
            --threads ${task.cpus}
        ...
        python3 ${moduleDir}/../bin/evaluate_trees.py \\
            ...
            --esttree true_msa_raxml_\${rep}.nwk \\
            --pipeline TRUE_MSA+RAXML \\
            --distance ${dist} \\
            --length ${len} \\
            --alpha ${params.alpha} \\
            --replicate \${rep} \\
            --json true_msa_raxml_meta_\${rep}.json \\
            --outcsv chunk_true_msa_raxml_D${dist}_L${len}_chk${chunk_id}.csv
    done
    """
}
