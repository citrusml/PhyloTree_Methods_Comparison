...
process RUN_TRUE_MSA_BI {
    ...
    output:
    path("chunk_true_msa_bi_D${dist}_L${len}_chk${chunk_id}.csv"), emit: csv
    path("true_msa_bi_*.nwk")
    ...
    for rep in \$(seq ${rep_start} ${rep_end}); do
        ...
        python3 ${moduleDir}/../bin/evaluate_trees.py \\
            ...
            --esttree true_msa_bi_\${rep}.nwk \\
            --pipeline TRUE_MSA+BI \\
            --distance ${dist} \\
            --length ${len} \\
            --alpha ${params.alpha} \\
            --replicate \${rep} \\
            --outcsv chunk_true_msa_bi_D${dist}_L${len}_chk${chunk_id}.csv
    done
    """
}
