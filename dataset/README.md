...
## 系統樹推定 & 分類学的整合性（TCS）ベンチマーク

本データセットの全 400 HOG に対する 4 手法（MSA+ML, MSA+NJ, PSA+NJ, GS）の系統樹構築（1,600本）、トポロジー一致度（RF / nRF）、および AmpliPhy 準拠の分類整合性スコア（TCS）解析の結果は、以下のドキュメントに網羅的にまとめられています：

* **詳細総括レポート**: [20260911_dataset_results.md](../memo/20260911_dataset_results.md)
* **主な結論**:
  - 全体 1 位は **最尤法（MSA+ML: 勝率 36.0%）**。
  - **GS法** は真核生物では低迷（5.0%）するが、原核生物（Actinobacteria: 35.1%, Bacteria: 33.2%）および遠縁帯（$0.2 \le \text{SSS} < 0.4$: 29.5%）でトップ勝率を獲得。
  - **PSA+NJ** は遠縁領域（$\text{SSS} < 0.4$）で MSA+NJ を勝率で一貫して凌駕。
  - 極限の超遠縁（$\text{SSS} < 0.1$）では、多重置換飽和により **NJ 法（PSA+NJ / MSA+NJ: 計 66.7% 勝率）が ML を逆転**。
