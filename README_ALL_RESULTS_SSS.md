...
#### 10. `results_high_gap` (広域距離 $D \in [0.1 .. 6.0]$ + FastME)
- **概要**: 近縁（$D=0.1$）から極遠縁（$D=6.0$）までを連続的に網羅し、FastME（SPR局所探索+LG+G補正 / デフォルト）を追加統合。
...
- **分析**:
  - 真のアラインメント対照群（`TRUE_MSA+ML`, `TRUE_PSA+NJ`）との比較により、アラインメント誤差と距離・探索誤差を分離評価。
  - `MSA+FastME (LG+G)`（黄緑実線）は $w > 0.10$ で `MSA+NJ` よりも低いトポロジー誤差（$nRF$）を達成し、SPR局所探索とガンマ距離補正の優位性を示しています。
  - 一方、極低類似度域（$w < 0.05$）ではアラインメントの破綻により、いずれの距離法（NJ, FastME）も同水準の誤差に漸近します。
